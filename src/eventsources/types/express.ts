...
export default class EventSource extends GSEventSource {
  async initClient(): Promise<PlainObject> {
    ...
    if (!process.env.VERCEL && !process.env.SERVERLESS) {
      ...
      logger.info("Running in serverless mode, not starting server.");
    }
  ...
  setupMiddleware(app: express.Express) {
    ...
    app.use(
      bodyParser.urlencoded({
        extended: true,
        limit: this.config.request_body_limit || '50mb',
      }),
    );
    ...
    app.use(
      fileUpload({
        useTempFiles: true,
        limits: { fileSize: this.config.file_size_limit || '50mb' },
        abortOnLimit: true,
      }),
    );
  ...
  setupGoogleAuthentication(app: express.Express, googleConfig: PlainObject) {
    ...
    const callbackRoute =
      googleConfig.callback_route || '/auth/google/callback';
  ...
  setupLinkedInAuthentication(
    app: express.Express,
    linkedinConfig: PlainObject,
  ) {
    if (
      !linkedinConfig.client_id ||
      !linkedinConfig.client_secret ||
      !linkedinConfig.callback_url
    ) {
    ...
    const callbackRoute =
      linkedinConfig.callback_route || '/auth/linkedin/callback';
    const failureRedirectURL = linkedinConfig.failure_redirect || '/error';
    const successRedirectURL =
      linkedinConfig.success_redirect || '/verify/user';
  ...
  setupGithubAuthentication(app: express.Express, githubConfig: PlainObject) {
    ...
    const callbackRoute =
      githubConfig.callback_route || '/auth/github/callback';
    ...
    app.get(
      callbackRoute,
      passport.authenticate('github', { failureRedirect: failureRedirectURL }),
      async (req, res) => {
        res.redirect(successRedirectURL);
      },
    );
  ...
  private authnHOF(authn: boolean) {
    return (
      ...
      if (
        authn !== false &&
        (this.config.authn?.oauth2 || this.config.authn?.oauth2?.google)
      ) {
